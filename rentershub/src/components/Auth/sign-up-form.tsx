...
import { User, Building, Mail, Lock, CheckCircle2, ArrowLeft, ArrowRight, EyeOff, Eye } from "lucide-react"
...
const SignUpForm = ({ initialRole }: SignUpFormProps) => {
  ...
  const fetchRoles = useCallback(async () => {
    try {
      ...
      // If initialRole is provided, set it and move to step 2
      if (initialRole) {
        const matchedRole = filteredRoles.find(
          (r: { role: string }) =>
            r.role.toLowerCase() === initialRole.toLowerCase() ||
            (initialRole.toLowerCase() === "groundagent" && r.role === "GROUNDAGENT") ||
            (initialRole.toLowerCase() === "landlord" && r.role === "LANDLORD"),
        )

        if (matchedRole) {
          setUserType(matchedRole.role.toLowerCase())
